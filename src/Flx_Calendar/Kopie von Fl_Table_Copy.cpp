//
// "$Id: Fl_Table_Copy.cxx 10225 2014-08-03 16:40:58Z greg.ercolano $"
//
// Fl_Table_Copy -- A table widget
//
...
void Fl_Table_Copy::col_position(int col) {
...
long Fl_Table_Copy::row_scroll_position(int row) {
  int startrow = 0;
  long scroll = 0; 
  // OPTIMIZATION: 
...
long Fl_Table_Copy::col_scroll_position(int col) {
  int startcol = 0;
  long scroll = 0;
  // OPTIMIZATION: 
  //     Attempt to use precomputed row scroll position
...
// Ctor
Fl_Table_Copy::Fl_Table_Copy(int X, int Y, int W, int H, const char *l) : Fl_Group(X,Y,W,H,l) {
  _rows             = 0;
  _cols             = 0;
  _row_header_w     = 40;
  _col_header_h     = 18;
...
// Dtor
Fl_Table_Copy::~Fl_Table_Copy() {
  // The parent Fl_Group takes care of destroying scrollbars
}

// Set height of a row
void Fl_Table_Copy::row_height(int row, int height) {
  if ( row < 0 ) return;
...
void Fl_Table_Copy::col_width(int col, int width)
{
  if ( col < 0 ) return;
...
int Fl_Table_Copy::row_col_clamp(TableContext context, int &R, int &C) {
  int clamped = 0;
...
void Fl_Table_Copy::get_bounds(TableContext context, int &X, int &Y, int &W, int &H) {
  switch ( context ) {
    case CONTEXT_COL_HEADER:
    ...
    default:
      fprintf(stderr, "Fl_Table_Copy::get_bounds(): context %d unimplemented\n", (int)context);
      return;
  }
  //NOTREACHED
}
...
void Fl_Table_Copy::_start_auto_drag() {
...
void Fl_Table_Copy::_stop_auto_drag() {
  if (!_auto_drag) return;
  Fl::remove_timeout(_auto_drag_cb2, this);
  _auto_drag = 0;
}

void Fl_Table_Copy::_auto_drag_cb2(void *d) {
  ((Fl_Table_Copy*)d)->_auto_drag_cb();
}

// Handle automatic scroll-selection if mouse selection dragged off table edge
void Fl_Table_Copy::_auto_drag_cb() {
  int lx = Fl::e_x;
  int ly = Fl::e_y;
...
//
void Fl_Table_Copy::table_scrolled() {
  // Find top row
...
void Fl_Table_Copy::scroll_cb(Fl_Widget*w, void *data) {
  Fl_Table_Copy *o = (Fl_Table_Copy*)data;
...
// Set number of rows
void Fl_Table_Copy::rows(int val) {
  int oldrows = _rows;
  _rows = val;
  {
...
// Set number of cols
void Fl_Table_Copy::cols(int val) {
  _cols = val;
  {
...
void Fl_Table_Copy::change_cursor(Fl_Cursor newcursor) {
...
void Fl_Table_Copy::damage_zone(int r1, int c1, int r2, int c2, int r3, int c3) {
  int R1 = r1, C1 = c1;
  int R2 = r2, C2 = c2;
  if (r1 > R2) R2 = r1;
  if (r2 < R1) R1 = r2;
...
int Fl_Table_Copy::move_cursor(int R, int C, int shiftselect) {
  if (select_row == -1) R++;
  if (select_col == -1) C++;
  R += select_row;
  C += select_col;
...
int Fl_Table_Copy::move_cursor(int R, int C) {
...
//
void Fl_Table_Copy::resize(int X, int Y, int W, int H) {
...
// Draw a cell
void Fl_Table_Copy::_redraw_cell(TableContext context, int r, int c) {
  if ( r < 0 || c < 0 ) return;
  int X,Y,W,H;
 ...
 */
int Fl_Table_Copy::is_selected(int r, int c) {
...
*/
void Fl_Table_Copy::get_selection(int& row_top, int& col_left, int& row_bot, int& col_right) {
  if (select_col > current_col) {
...
*/
void Fl_Table_Copy::set_selection(int row_top, int col_left, int row_bot, int col_right) {
  damage_zone(current_row, current_col, select_row, select_col);
...
//
// End of "$Id: Fl_Table_Copy.cxx 10225 2014-08-03 16:40:58Z greg.ercolano $".
//
